...
mod tests {
    ...
    use crate::poly_utils::{coeffs::CoefficientList, eq_poly_outside, MultilinearPoint};
    ...
    fn test_sumcheck_folding_factor_1() {
        ...
        let eval_point = MultilinearPoint(vec![F::from_canonical_u64(10), F::from_canonical_u64(11)]);
        let polynomial =
            CoefficientList::new(vec![F::from_canonical_u64(1), F::from_canonical_u64(5), F::from_canonical_u64(10), F::from_canonical_u64(14)]);
        ...
        let folding_randomness = MultilinearPoint(vec![F::from_canonical_u64(4999)]);
    ...
    fn test_single_folding() {
        ...
        let polynomial = CoefficientList::new(vec![F::from_canonical_u64(1), F::from_canonical_u64(2), F::from_canonical_u64(3), F::from_canonical_u64(4)]);

        let ood_point = MultilinearPoint::expand_from_univariate(F::from_canonical_u64(2), num_variables);
        let statement_point = MultilinearPoint::expand_from_univariate(F::from_canonical_u64(3), num_variables);

        let ood_answer = polynomial.evaluate(&ood_point);
        let statement_awnswer = polynomial.evaluate(&statement_point);
        ...
        assert_eq!(
            ...
            epsilon_1 * ood_answer + epsilon_2 * statement_awnswer
        );

        let folding_randomness = MultilinearPoint(vec![F::from_canonical_u64(400000), F::from_canonical_u64(800000)]);
    ...
    fn test_sumcheck_folding_factor_2() {
        let num_variables = 6;
        let folding_factor = 2;
        let eval_point = MultilinearPoint(vec![F::from_canonical_u64(97); num_variables]);
        let polynomial = CoefficientList::new((0..1 << num_variables).map(F::from_canonical_u64).collect());

        let claimed_value = polynomial.evaluate(&eval_point);

        let mut prover = SumcheckCore::new(polynomial.clone(), &[eval_point], &[F::from_canonical_u64(1)]);
        ...
        let combination_randomness = vec![F::from_canonical_u64(293), F::from_canonical_u64(42)];
        let folding_randomness = MultilinearPoint(vec![F::from_canonical_u64(335), F::from_canonical_u64(222)]);

        let new_eval_point = MultilinearPoint(vec![F::from_canonical_u64(32); num_variables - folding_factor]);
    ...
    fn test_e2e() {
        ...
        // Initial stuff
        let ood_point = MultilinearPoint::expand_from_univariate(F::from_canonical_u64(42), num_variables);
        let statement_point = MultilinearPoint::expand_from_univariate(F::from_canonical_u64(97), num_variables);

        // All the randomness
        let [epsilon_1, epsilon_2] = [F::from_canonical_u64(15), F::from_canonical_u64(32)];
        let folding_randomness_1 = MultilinearPoint(vec![F::from_canonical_u64(11), F::from_canonical_u64(31)]);
        let fold_point = MultilinearPoint(vec![F::from_canonical_u64(31), F::from_canonical_u64(15)]);
        let combination_randomness = vec![F::from_canonical_u64(31), F::from_canonical_u64(4999)];
        let folding_randomness_2 = MultilinearPoint(vec![F::from_canonical_u64(97), F::from_canonical_u64(36)]);
    ...
    fn test_e2e_larger() {
        ...
        let folding_randomness_1 = MultilinearPoint(vec![F::from_canonical_u64(11), F::from_canonical_u64(31)]);
        let folding_randomness_2 = MultilinearPoint(vec![F::from_canonical_u64(97), F::from_canonical_u64(36)]);
        let folding_randomness_3 = MultilinearPoint(vec![F::from_canonical_u64(11297), F::from_canonical_u64(42136)]);
        let fold_point_11 =
            MultilinearPoint(vec![F::from_canonical_u64(31), F::from_canonical_u64(15), F::from_canonical_u64(31), F::from_canonical_u64(15)]);
        let fold_point_12 =
            MultilinearPoint(vec![F::from_canonical_u64(1231), F::from_canonical_u64(15), F::from_canonical_u64(4231), F::from_canonical_u64(15)]);
        let fold_point_2 = MultilinearPoint(vec![F::from_canonical_u64(311), F::from_canonical_u64(115)]);
        let combination_randomness_1 = vec![F::from_canonical_u64(1289), F::from_canonical_u64(3281), F::from_canonical_u64(10921)];
        let combination_randomness_2 = vec![F::from_canonical_u64(3281), F::from_canonical_u64(3232)];
